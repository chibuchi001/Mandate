...
import { getTokenFromVault, googleCalendarList, slackPost, githubListPRs } from "./token-vault";
...
async function executeReal(
  ...
  switch (action.operation) {
    ...
    case "calendar_find_free_slots": {
      ...
      const data = await googleCalendarList(auth0Token, now.toISOString(), tomorrow.toISOString());
    ...
    case "slack_post_message": {
      ...
      const data = await slackPost(auth0Token, channel, text);
    ...
    case "github_review_pr": {
      ...
      const res = await fetch(`https://api.github.com/repos/${repo}/pulls/${pr}/reviews`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token.access_token}`, Accept: "application/vnd.github.v3+json", "Content-Type": "application/json" },
